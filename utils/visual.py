...
def draw_lines(image, lines):
    ...
    for line in lines:
        ...
        # 点
        if len_of_points == 1:
            color = points[0].get('color', attribute.get('color', [0, 255, 0]))
            point_xy = points[0]['coordinate']
            radius = int(min(h, w) * 0.005)
            image_draw.ellipse(xy=(point_xy[0] - radius, point_xy[1] - radius,
                                   point_xy[0] + radius, point_xy[1] + radius), fill=tuple(color))
            if not text:
                text = points[0].get('text', attribute.get('text', ''))
                image_draw.text(xy=(point_xy[0] + radius, point_xy[1] + radius), text=text, fill=tuple(color))
            continue
        ...
        for i in range(len(points) - 1):
            ...
            if not text:
                text = points[i].get('text', attribute.get('text', ''))
                image_draw.text(xy=tuple(line_i[0]), fill=tuple(color), text=text)
